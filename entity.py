...
class Entity(object):
    ...
    def __execute_query(self, query, args):
        ...
        except Exception(error_arg):
            self.__class__.db.rollback()
            raise DatabaseError(error_arg)
    ...
    def __update(self):
        ...
        c
        self.__execute_query(
            self.__update_query.format(
                table='%s',
                columns='%s'
            ),
            (self.__table, col, self.__id,)
        )
    ...
    def _get_children(self, name):
        ...
        self.__execute_query(
            self.__parent_query.format(
                table=self._children[name].lower(),
                parent=self.__table
            ),
            (self.__id,)
        )
        ...
        for row in rows:
            child_cls = getattr(importlib.import_module("models"), (self._children[name]))
            instance = child_cls()
            instance.__id = row['{table}_id'.format(table=instance.__table)]
    ...
    def _get_parent(self, name):
        ...
        parent_cls = getattr(importlib.import_module("models"), (name.title()))
        instance = parent_cls(parent_id)
    ...
    def _get_siblings(self, name):
        ...
        for row in rows:
            sibling_cls = getattr(importlib.import_module("models"), (name.title()))
            instance = sibling_cls()
            instance.__id = row['{table}_id'.format(table=instance.__table)]
            instance.__fields = dict(row)
            instance.__loaded = True
            instances.append(instance)
    ...
    def all(cls):
        ...
        print(cls_obj.__list_query.format(table=cls_obj.__table))
